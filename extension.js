...
function activate(context) {
    ...
    let exportDisposable = vscode.commands.registerCommand("cursor.exportSettings", async () => {
        try {
            ...
            vscode.window.showInformationMessage("Cursor settings exported successfully");
        }
        catch (error) {
    ...
    let importDisposable = vscode.commands.registerCommand("cursor.importSettings", async () => {
        try {
            ...
            vscode.window.showInformationMessage("Cursor settings imported successfully");
        }
        catch (error) {
    ...
    let syncDisposable = vscode.commands.registerCommand("cursor.syncSettings", async () => {
        try {
            await exportCursorSettings();
            await importCursorSettings();
            vscode.window.showInformationMessage("Cursor settings synced successfully");
        }
        catch (error) {
            vscode.window.showErrorMessage(`Failed to sync settings: ${error}`);
        }
    });
    context.subscriptions.push(ignoreDisposable, focusDisposable, exportDisposable, importDisposable, syncDisposable);
}
